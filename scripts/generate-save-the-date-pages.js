...
const DATA_FILE = path.join(ROOT, "assets", "save-the-date-templates-data.js");
const SHELL_FILE = path.join(__dirname, "save-the-date-product-page-shell.html");
const OUTPUT_DIR = path.join(ROOT, "save-the-date");

function loadTemplates() {
  const source = fs.readFileSync(DATA_FILE, "utf8");
  const sandbox = { window: {}, global: {} };
  sandbox.global = sandbox.window;
  const fn = new Function("window", "global", source + "\n;return window.DvitesSaveTheDateTemplates;");
  const api = fn(sandbox.window, sandbox.global);
  if (!api || !Array.isArray(api.list) || !api.list.length) {
    throw new Error("No save-the-date templates found in assets/save-the-date-templates-data.js");
  }
  return api.list;
}
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Save the Date", item: "https://www.dvites.com/save-the-date.html" },
...
function renderPage(shell, tpl) {
  return shell
    ...
    .replace(/\{\{PRICE\}\}/g, String(tpl.price))
    .replace(/\{\{OLD_PRICE\}\}/g, String(tpl.oldPrice))
    .replace(/\{\{SAVE_LABEL\}\}/g, tpl.saveLabel);
}
...
function main() {
  ...
  templates.forEach(function (tpl) {
    ...
    written.push("/save-the-date/" + tpl.slug + ".html");
  });

  console.log("Generated " + written.length + " save-the-date template product pages:");

...
const EVENTS = [
  {
    id: "mehendi",
    icon: "./assets/images/event/pn-evt-ico-mehendi-x-v01.webp",
    name: "Mehendi",
  ...
  {
    id: "haldi",
    icon: "./assets/images/event/pn-evt-ico-haldi-x-v01.webp",
    name: "Haldi",
  ...
  {
    id: "sangeet",
    icon: "./assets/images/event/pn-evt-ico-sangeet-x-v01.webp",
    name: "Sangeet",
  ...
  {
    id: "shaadi",
    icon: "./assets/images/event/pn-evt-ico-shaadi-x-v01.webp",
    name: "Shaadi",
  ...
  {
    id: "reception",
    icon: "./assets/images/event/pn-evt-ico-reception-x-v01.webp",
    name: "Reception",
  ...
  {
    id: "vidaai",
    icon: "./assets/images/event/pn-evt-ico-vidaai-x-v01.webp",
    name: "Vidaai",
...
───────────────────────────────────── */
const A = {
  darkBg:      "./assets/images/hero/pn-hro-bg-courtyard-dark-m-v03.webp",
  litBg:       "./assets/images/hero/pn-hro-bg-courtyard-lit-m-v03.webp",
  darkBgDesktop: "./assets/images/hero/pn-hro-bg-courtyard-dark-D-v03.webp",
  litBgDesktop:  "./assets/images/hero/pn-hro-bg-courtyard-lit-d-v03.webp",
  rope:        "./assets/images/hero/pn-hro-el-rope-hemp-pull-x-v01.webp",
  lotusClosed: "./assets/images/hero/pn-rvl-btn-lotus-closed-x-v01.webp",
  lotusOpen:   "./assets/images/hero/pn-rvl-btn-lotus-open-x-v01.webp",
  lotusGlow:   "./assets/images/hero/pn-fx-ovl-lotus-glow-burst-x-v01.webp",
  jhoomer:     "./assets/images/shared/pn-shr-mot-jhoomer-hanging-x-v01.webp",
  floralBush:  "./assets/images/hero/pn-shr-mot-floral-bush-cluster-x-v01.webp",
  diya:        "./assets/images/hero/pn-shr-mot-diya-glow-x-v01.webp"
};
  ...
  EVENTS.forEach((evt, i) => {
    ...
    article.innerHTML = `
      <div class="farman-rolled-wrap" aria-hidden="true">
        <img class="farman-rolled-img"
             src="./assets/images/event/pn-evt-farman-rolled-x-v01.webp"
             alt="" draggable="false" decoding="async">
      </div>
      <div class="farman-open-wrap">
        <img class="farman-parchment-img"
             src="./assets/images/event/pn-evt-farman-open-x-v01.webp"
    ...
    if (i < EVENTS.length - 1) {
      ...
      if (i === LOTUS_AFTER_INDEX) {
        ...
        inter.innerHTML = `<img src="./assets/images/invite/pn-inv-div-lotus-divider-x-v01.webp"
...
function renderStars() {
  ...
  function draw(ms) {
    ...
    for (const s of stars) {
      ...
      /* Soft glow halo on larger stars only — skipped on mobile, where the
         per-star radial gradient each frame is a heavy GPU/memory cost. */
      if (s.r > 1.15 && !mobile) {
...
const GAL_FRAMES = {
  landscape: "./assets/images/gallery/pn-gal-fr-hanging-landscape-x-v01.webp",
  portrait:  "./assets/images/gallery/pn-gal-fr-hanging-portrait-x-v01.webp",
  hero:      "./assets/images/gallery/pn-gal-fr-hanging-hero-arch-x-v01.webp",
};
...
(function initPetalShower() {
  ...
  const MAX_PETALS        = 8;
  const PETAL_SRC         = './assets/images/invite/petal.webp';
  ...
  function resize() {
    /* Cap backing-store density — full retina (2–3×) blows the per-tab
       memory budget on old iPhones with no visible benefit for petals. */
...
const TTK_BASE = "./assets/images/ttk/";
...
/* ═══════════════════════════════════════════════════════════════
   DVITES — Phase 1 config override (runs before hydrate)
...
/* ═══════════════════════════════════════════════════════════════
   DVITES — Phase 2 config override (runs after all init)
...
(function applyConfigPhase2() {
  ...
  if (couple.bride && couple.groom) {
    document.title = couple.bride + ' & ' + couple.groom + ' · Dvites';
  }
  ...
  /* ── Invite — opening blessing (shown when grandparents are off) ──
     Keep the template's static 3-line default untouched; only override
     lines 1–2 when the user customised the blessing. Line 3 ("Together
     we invite you to celebrate") stays as the fixed connector. */
  var DEFAULT_BLESSING = 'With the blessings of the divine and the love of our families';
  var line1El = document.querySelector('.inv-blessing-alt-line1');
  var line2El = document.querySelector('.inv-blessing-alt-line2');
  if (line1El && line2El && invite.blessing && invite.blessing.trim() !== DEFAULT_BLESSING) {
    function escBless(s) {
      return String(s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    }
    var parts = String(invite.blessing).split('\n');
    line1El.innerHTML = escBless(parts[0] || '');
    if (parts.length > 1) {
      line2El.innerHTML = parts.slice(1).map(escBless).join('<br>');
  ...
  /* ── Story hashtag — show user's wedding hashtag, preserving their case ── */
  var hashtagEl = document.getElementById('cplHashtag');
  if (hashtagEl) {
    var htag = (story.customHashtag || couple.hashtag || '').trim();
    if (htag) {
      if (htag.charAt(0) !== '#') htag = '#' + htag;
      hashtagEl.textContent = htag;
      hashtagEl.removeAttribute('hidden');
    }
  }
  ...
  /* ── Calendar links — use Dvites pipeline URLs ── */
  if (C.calendarUrls) {

...
const URL = "http://127.0.0.1:5500/templates/niqah/index.html";
const PORT = 5500;
...
async function main() {
  ...
  const hasInviteText =
    (await page.locator("text=FARDEEN").count()) > 0 ||
    (await page.locator("text=ZARIN").count()) > 0 ||
    (await page.locator("text=/wedding celebrations/i").count()) > 0 ||
    (await page.locator("text=INVITE").count()) > 0 ||
    (await page.locator("text=Nikah").count()) > 0;
